...
if ($_FILES['user_photo']['error'] === UPLOAD_ERR_OK) {
    ...
    $imageUploadDirectory = 'Uploads/Profile/PGStudent/';
    ...
    if (in_array($fileExtension, $allowedExtensions) && $fileSize <= $maxFileSize) {
        ...
        if (move_uploaded_file($fileTmpPath, $uploadPath)) {
            ...
            echo ("<script>window.location = 'studentpgform.php';</script>");
        }
    } else {
...
if ($_FILES['user_file1']['error'] === UPLOAD_ERR_OK) {
    ...
    $pdfUploadDirectory = 'Uploads/Suploads/PG/';
...
if ($_FILES['user_file4']['error'] === UPLOAD_ERR_OK) {
    ...
    // You may want to generate a unique filename to prevent overwriting files with the same name
    $newFileName = 'PG_Marksheet_' . $rollno . '.' . $fileExtension;
...
if ($_FILES['user_file5']['error'] === UPLOAD_ERR_OK) {
    $fileTmpPath = $_FILES['user_file5']['tmp_name'];
    $fileName = $_FILES['user_file5']['name'];
    $fileSize = $_FILES['user_file5']['size'];
    $fileType = $_FILES['user_file5']['type'];
    $fileNameCmps = explode(".", $fileName);
    $fileExtension = strtolower(end($fileNameCmps));
    ...
    if (in_array($fileExtension, $allowedExtensions) && $fileSize <= $maxFileSize) {
        ...
        if (move_uploaded_file($fileTmpPath, $uploadPath)) {
            $pdfUploadPath5 = $uploadPath;
            // PDF file uploaded successfully

            // Insert user data into the database
            $sql = "INSERT INTO studentpgform (name, rollno, dob, age, phone, email, fname, mname, peradd, city, pincode, state, country, course, dept, school1, school2, college, tenth, twelveth, ug, pg, photo_path, file_path1, file_path2, file_path3, file_path4, file_path5) VALUES ('$name', '$rollno', '$dob', $age, $phone, '$email', '$fname', '$mname', '$peradd',  '$city', '$pincode', '$state', '$country', '$course', '$dept', '$school1', '$school2', '$college', '$tenth', '$twelveth', '$ug', '$pg', '$imageUploadPath',  '$pdfUploadPath1', '$pdfUploadPath2', '$pdfUploadPath3', '$pdfUploadPath4', '$pdfUploadPath5')";
